...
void ScreenNowPlaying::update(const Player& player)
{
	...
	{
		s += "/" + std::to_string((int)(player.duration() / 60.0f)) + ":";
	...
	if (player.playlistSize() == 0)
		...
		s = std::to_string(player.playlistIndex() + 1) + "/" +  std::to_string(player.playlistSize());
	mPlaylist.move(width() - s.length() + 1);
	mPlaylist.setText(s);
	...
	{
		mLine2.setText("");
		mLine3.setText(string(width() > cEmptyQueueTextLength ? width() / 2 - cEmptyQueueTextLength / 2 : 0, ' ') + cEmptyQueueText);
		mLine4.setText("");
		return;
	}
	...
	mLine4.setText(trail(toLCDString(player.title())));
}
